...
// Teacher Payment History page (dedicated sidebar tab)
...
const TeacherPaymentHistory = () => {
  ...
  const downloadPdf = async (payment) => {
    try {
      ...
      a.href = url;
      a.download = `vidyalaya-payment-record-${payment.transactionId || payment.pidx || payment._id}.pdf`;
              ...
              <div className="flex items-start justify-between gap-4 mb-5">
                <div>
                  ...
                  <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
                    Payments received for your courses (Khalti).
                  </p>
                </div>
              </div>
                      ...
                      >
                        <div className="min-w-0">
                          ...
                          <div className="mt-1 flex flex-wrap items-center gap-3 text-xs text-slate-500 dark:text-slate-400">
                            <span className="inline-flex items-center gap-1">
                              <FaUserGraduate className="text-[12px] text-amber-500" />
                        ...
                        <div className="flex items-center gap-2">
                          <button
                            ...
                            className="px-3 py-2 rounded-lg text-sm font-medium bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-600 text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors"
                          >
                            View course
                          </button>
                          ...
                          <button
                            ...
                            className="px-3 py-2 rounded-lg text-sm font-semibold bg-amber-500 text-white hover:bg-amber-600 disabled:opacity-50 disabled:cursor-not-allowed transition-colors inline-flex items-center gap-2"
                            title={p.status !== 'completed' ? 'PDF is available only for completed payments' : 'Download PDF'}
                          >
